...
이 프로젝트는 C 언어의 서브셋을 x86-64 어셈블리 코드로 변환하는 Mini C 컴파일러를 구현합니다.
...
1.  **저장소 복제:**
    ...
    ```
2.  **컴파일러 빌드:**
    `make all`을 실행하여 `minic_x86` 실행 파일을 빌드합니다. 이 명령어는 어휘 분석기와 파서 소스 파일을 생성하고, 모든 구성 요소를 컴파일한 후 최종 컴파일러 실행 파일로 링크합니다.
    ```bash
    make all
    ```
...
├── include/
│   ├── ast.h               # AST 노드 구조체 정의
│   └── codegen_x86.h       # 코드 생성기 함수 선언
├── parser/
│   ├── lexer.l             # 어휘 분석을 위한 Flex 파일
│   └── parser.y            # 파싱을 위한 Bison 파일
├── src/
│   ├── ast.c               # AST 생성 및 관리 함수
│   ├── codegen_x86.c       # x86-64 어셈블리 코드 생성기
│   └── main.c              # 컴파일러 메인 드라이버
├── test/
│   └── ...                 # 테스트 소스 파일
└── ...
